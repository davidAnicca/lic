...
def validate(G, P, M, K, E, input_hp, bounds: dict):
    ...
    if not (bounds["G_MIN"] <= G <= bounds["G_MAX"]):
        errs.append(f"Generations number {G} is outside [{bounds['G_MIN']}, {bounds['G_MAX']}]")

    if not (bounds["P_MIN"] <= P <= bounds["P_MAX"]):
        errs.append(f"Population size {P} is outside [{bounds['P_MIN']}, {bounds['P_MAX']}]")
    ...
    if K < bounds["K_MIN"] or K >= P:
        errs.append(f"Tournament size {K} invalid: must be >= {bounds['K_MIN']} and < {P}")
